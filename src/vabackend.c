...
static __attribute__((unused)) void parseSurfaceImportAttributes(VASurfaceAttrib *attribList, unsigned int numAttribs, ImportedSurface *imported) {
...
static __attribute__((unused)) BackingImage *createImportedBackingImage(NVDriver *drv, const ImportedSurface *imported, uint32_t width, uint32_t height) {
        ...
        )
{
    ...
    const uint32_t surfaceFourcc = importExternalPrime2
        ? externalSurfaceDesc->fourcc
        : (importExternalPrime ? externalBufferDesc->pixel_format : 0);
...
static __attribute__((unused)) bool copySurfaceBackingImage(NVDriver *drv, NVSurface *src, NVSurface *dst, const VAProcPipelineParameterBuffer *pipeline) {
